...
export default function MyInfoContent() {
    const { session, isLoading, profiles, categories, userPosts, commentCountMap } =
        useMyInfoData();
    ...
    const isEditor = profiles.length > 0 && profiles.some((p) => p.role === "edit");
    const currentBanner = profiles.length > 0 && profiles[0]?.profile_banner
